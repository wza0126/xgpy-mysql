-- 修复刷题打怪系统的数据库字段
-- 019 迁移的多列 ALTER TABLE 未被迁移处理器正确处理，需要使用单列 ALTER TABLE

-- 1. 逐个添加游戏字段
ALTER TABLE profiles ADD COLUMN IF NOT EXISTS curr_streak INT NOT NULL DEFAULT 0 COMMENT '当前连续答对题数';
ALTER TABLE profiles ADD COLUMN IF NOT EXISTS curr_crit_streak INT NOT NULL DEFAULT 0 COMMENT '当前连续暴击次数';
ALTER TABLE profiles ADD COLUMN IF NOT EXISTS curr_wrong INT NOT NULL DEFAULT 0 COMMENT '当前连续答错次数';
ALTER TABLE profiles ADD COLUMN IF NOT EXISTS perfect_10_times INT NOT NULL DEFAULT 0 COMMENT '累计达成十连对次数';
ALTER TABLE profiles ADD COLUMN IF NOT EXISTS triple_crit_times INT NOT NULL DEFAULT 0 COMMENT '累计达成三连爆次数';
ALTER TABLE profiles ADD COLUMN IF NOT EXISTS wrong_3_times INT NOT NULL DEFAULT 0 COMMENT '累计三连错次数';
ALTER TABLE profiles ADD COLUMN IF NOT EXISTS extra_crit DECIMAL(5,2) NOT NULL DEFAULT 0 COMMENT '副本通关赠送暴击值(%)';
ALTER TABLE profiles ADD COLUMN IF NOT EXISTS total_correct INT NOT NULL DEFAULT 0 COMMENT '历史答对总题数';
ALTER TABLE profiles ADD COLUMN IF NOT EXISTS pet_level INT NOT NULL DEFAULT 1 COMMENT '宠物等级';

-- 2. 确保 student_buffs 表存在（以防 019 迁移也失败了）
