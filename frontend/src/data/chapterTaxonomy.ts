 ...
 * 考点章节词表（前端只读副本）
 *
 * 【唯一数据源】backend/src/chapter-taxonomy.js —— 改词表请改那边，再同步此文件。
 * 前端保持副本的原因：教师端「人工改挂」下拉框需要离线渲染章节树；
 * 学生端学习模块目录仍通过 /api/knowledge/chapters 从后端拉取。
 *
 * 结构：4 大部分 / 16 章 / 70 小节
 *  - 一级分类 = 章（chapter），如「数据与信息」
 *  - 二级分类 = 小节（section），如「数据编码」下的「文本数据的编码」
 *  - allowChapter = true 的章允许题目只挂到章级（讲义未细分的考点兜底）
 */
...
export const CHAPTER_TAXONOMY: TaxonomyPart[] = [
  {
    part: '必修一',
    partName: '数据与计算',
    group: '第一部分 数据',
...
/** 所有章名 */
export const CHAPTER_NAMES: string[] = CHAPTER_TAXONOMY.flatMap((p) => p.chapters.map((c) => c.name));

/** 所有「章名/小节名」路径 */
export const SECTION_PATHS: string[] = CHAPTER_TAXONOMY.flatMap((p) =>
  p.chapters.flatMap((c) => c.sections.map((s) => `${c.name}/${s}`))
);

/** 章名 -> 是否允许题只挂章级 */
export const CHAPTER_ALLOW_SELF: Record<string, boolean> = Object.fromEntries(
  CHAPTER_TAXONOMY.flatMap((p) => p.chapters.map((c) => [c.name, !!c.allowChapter]))
);

/** 校验一个 cluster_id 是否合法（不在词表内返回 false） */
export function isValidClusterId(id: string): boolean {
  if (!id) return false;
  if (id === '其他') return true;
  if (SECTION_PATHS.includes(id)) return true;
  if (CHAPTER_NAMES.includes(id)) return !!CHAPTER_ALLOW_SELF[id];
  return false;
}

export function splitCluster(clusterId: string): { primary: string; secondary: string } {
  const idx = clusterId.indexOf('/');
  if (idx === -1) return { primary: clusterId, secondary: '' };
  return { primary: clusterId.slice(0, idx), secondary: clusterId.slice(idx + 1) };
}
