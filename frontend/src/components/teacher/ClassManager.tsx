                ...
                <td className="px-6 py-4 text-center">
                  {editingClass?.id === cls.id ? (
                    <label className="flex items-center justify-center gap-2 cursor-pointer">
                      <input
                        type="checkbox"
                        checked={isClassBoolEnabled(editingClass!, 'pk_battle_enabled')}
                        onChange={(e) => setEditingClass({ ...editingClass!, pk_battle_enabled: e.target.checked })}
                        className="w-5 h-5 rounded border-gray-300 text-blue-600 focus:ring-blue-500"
                      />
                      <span className="text-sm text-gray-600">
                        {isClassBoolEnabled(editingClass!, 'pk_battle_enabled') ? '启用' : '禁用'}
                      </span>
                    </label>
                  ) : (
                    <button
                      onClick={() => toggleClassQuickSwitch(cls, 'pk_battle_enabled')}
                      className={`px-4 py-2 rounded-full text-sm font-medium transition-all ${
                        isClassBoolEnabled(cls, 'pk_battle_enabled')
                          ? 'bg-green-100 text-green-700 hover:bg-green-200'
                          : 'bg-gray-100 text-gray-500 hover:bg-gray-200'
                      }`}
                    >
